...
We've got a pretty messy file to read in. Never fear - between `stringr` and `janitor` we can make short work of this. We're using `readr::read_file()` to read in the text as is.
...
We need to implement some validation rules. `pointblank` allows for custom data validation:
